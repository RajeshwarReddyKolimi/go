...
func main() {
	...
	testDb := NewDb(*dbName)
	err = testDb.db.AutoMigrate(&Author{}, &Blog{})
	if err != nil {
	...
	blog1, err := testDb.CreateBlog(Blog{Title: "John's Blog", Content: "This is a blog written by john", AuthorId: uint(john.ID)})
	if err != nil {
	...
	blog2, err := testDb.CreateBlog(Blog{Title: "Alice's Blog", Content: "This is a blog written by alice", AuthorId: uint(alice.ID)})
	if err != nil {
		fmt.Println("Error creating blog: ", err)
		return
	}
	...
	err = testDb.db.Migrator().DropTable(&Blog{}, &Author{})
	if err != nil {
